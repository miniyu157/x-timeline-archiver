...
- 针对于 X 页面的虚拟节点，提供自动滚动功能便捷获取 **时间线 (Timeline)** 与 **帖子树 (Thread)**；
- 输出格式为 JSON(L) 或 CSV，
基础数据包含各项互动指标（点赞、转贴、查看等）、帖子上下文、图文内容，并自动解析引用帖子；  
...
- 原本想要获取并解析完整的推文树，但似乎非常困难且容易造成账号枫红。所以目前仍需手动点击进入回复详情页，来查看或归档特定的子回复。
